...
Todos os arquivos relacionados a esse projeto podem ser encontrados neste [endereço](https://github.com/rodolfoksveiga/saint_gobain).

***

### Dependências

Para reproduzir o que foi descrito nesse documento é necessário que o usuário possua em sua máquina a linguagem [R](https://www.r-project.org/) instalada e os seguintes arquivos **localizados no mesmo diretório**:

1. [predict_outputs.r](https://drive.google.com/uc?export=download&id=1kCJYMu2rNBbtG7N4BvmD7ugDFKxosLOD)
2. [models.rds](https://drive.google.com/uc?export=download&id=12qCwPNFevstfo89JdRuIPf-eMWDOQ1lc)
...
Antes de qualquer operação carregue os pacotes que serão utilizados durante a execução do código e os metamodelos que serão utilizados para realizar as predições. Caso os arquivos *predict_outputs.r* e *models.rds* não estejam no mesmo diretório, um erro interromperá a execução do código.
...
**As mesmas mensagens que apareceram acima, após o carregamento dos pacotes, devem surgir no seu console caso os pacotes de fato tenham sido carregados no seu ambiente de trabalho.**
...
Para cada tipologia e cada variável de saída há um metamodelo (totalizando 8 metamodelos). A função `PredictOutput()`, que seleciona o metamodelo correspondente à variável de saída e à tipolgia desejadas e realiza as predições variável de interesse possui 3 argumentos: a) `targ` é a variável a ser predita ("phft", "dif_phft", "cgtt" ou "dif_cgtt"); b) dummies é o conjunto de dados de entrada após as transformação das variáveis qualitativas em variáveis lógicas; c) `typo` é a tipologia da edificação ("uni" ou "multi"). Após executada, `PredictOutput()` retorna um vetor com os valores da predições.
...
Defina uma lista de variáveis de entrada com apenas um valor para cada variável de entrada e chame a função `PredPerfUH()`. Associe o argumento `inputs` à lista pré-definida no seu ambiente de trabalho.
...
Note que, para valores únicos, apenas os outputs são armazenados.
...
Defina o caminho (na máquina local) para o arquivo *csv* com os *inputs* e chame a função `PredPerfUH()`, associando o caminho pré-definido ao argumento `inputs`.
...
Repare que a tabela de *inputs* inserida nesse exemplo possui 27668 observações (linhas), portanto, imprima apenas as 20 primeiras linhas.
...
A função `PredPerfUH()` salva automaticamente os resultados de forma padronizada no mesmo diretório onde encontram-se os arquivos **predict_outputs.r** e **models.rds**. Entretanto, o argumento `data_path` permite que se salve os resultados com um nome alternativo ou, ainda, um caminho alternativo até outro diretório. Se não for do interesse do usuário, é possível evitar que os resultados sejam salvos.
...
Simplesmente execute a função `PredPerfUH()` sem preencher o argumento `data_path`.
...
É possível definir um nome de arquivo personalizado preenchendo o argumento `data_path` com o nome desejado. Esse arquivo será salvo no mesmo diretório onde estão os arquivos **predict_outputs.r** e **models.rds**. Lembre-se que o nome arquivo de saída deve possuir a extensão ***csv***.

Execute `PredPerfUH()` com um nome alternativo.
...
Execute `PredPerfUH()` com um caminho alternativo.
...
Por fim, é possível orientar a função `PredPerfUH()` a não salvar o arquivo *csv*. Para tanto, defina o argumento `data_path` como `data_path = NULL`.
